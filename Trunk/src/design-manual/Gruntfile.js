module.exports = function(grunt) {
  ...
  grunt.initConfig({
	...
	uglify: {
	  options: {
		banner: '/*! ICAEW design-manual <%= grunt.template.today("yyyy-mm-dd") %> */\n'		// Change to application specific name
	  },
	  frameworksdebug: {
		options: {
			beautify:true
		...
		dest: '../../debug/js/design-manual/frameworks.js'
	  },	  
	  frameworks: {
		src: [
			// '../../js/library/modernizr.js',
		...
		dest: '../../production/js/design-manual/frameworks.js'
	  },	  	  
	  applicationdebug: {
		options: {
			beautify:true
		...
		src: [
			...
			// '../../js/design-manual/swatch-hex.js'
			// '../../js/global/menuMedium.js'
		],
		dest: '../../debug/js/design-manual/application.js'		// Change to application specific directory
	  },
	  application: {
		src: [
			'bower_components/foundation/js/foundation/foundation.js',
		...
		dest: '../../production/js/design-manual/application.js'		// Change to application specific directory
	  } 
	},
	compass: {
		development: {
	...
	stencil: {
		main: { // task target name
		  ...
		  files: [{
			...
			cwd: 'pages/',
			src: '**/*.dot.html',
			dest: '../../',				// Change to application specific directory
			ext: '.html'
		  }]
		}
	},
...
// Amend loaded grunt plugins from node_modules and grunt tasks as necessary.  New node_modules may only be added to the global library after assessment
  ...
  // Build tasks
  grunt.registerTask('debug', ['uglify','svgmin','compass:development','stencil']);
  grunt.registerTask('release', ['uglify','svgmin','compass:production']);
